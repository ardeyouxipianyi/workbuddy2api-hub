/* An account action must not move the panel to the other realm's view.
   ...
   which snaps VIEW_REALM back to the active gateway exit - so with the exit set
   to the other realm, the action threw the panel across. Run with Node.
*/
...
const api = new Function(script + `
  ...
  // initRealm() can reach updateUI() from inside the stub.
...
(async () => {
  ...
  // 2. keepView refreshes the exit but leaves the view alone.
  activeRealm = 'intl';
  window.VIEW_REALM = 'cn';
  window.ACTIVE_GATEWAY_REALM = 'cn';      // stale local copy of the exit
  await api.initRealm(true);
  assert.equal(window.ACTIVE_GATEWAY_REALM, 'intl', 'the exit is still refreshed');
  assert.equal(window.VIEW_REALM, 'cn', 'keepView must not move the view');

  // 3. The reported bug: 签到 from the cn view while the exit is intl.
  activeRealm = 'intl';
  window.VIEW_REALM = 'cn';
  await api.doCheckin(button());
  assert.equal(window.VIEW_REALM, 'cn', 'doCheckin must not jump to the intl view');

  // 4. The symmetric case: 活跃打卡 from the intl view while the exit is cn.
  activeRealm = 'cn';
